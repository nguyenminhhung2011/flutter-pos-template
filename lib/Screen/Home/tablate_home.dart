...
class _TablateHomeState extends State<TablateHome> {
  ...
  List<TopCustomer> getTopCustomer(List<CustomerModel> model) {
    ...
    model.reversed.toList().forEach((element) {
      TopCustomer report = TopCustomer(element.customerName, element.dueAmount, element.phoneNumber, element.profilePicture);
      customers.add(report);
    });
    return customers;
  }
  ...
  List<TopSellReport> getTopSellingReport(List<ProductModel> model) {
    ...
    model.reversed.toList().forEach((element) {
      TopSellReport report = TopSellReport(element.productName, element.productSalePrice, element.productCode,element.productStock, element.productPicture);
      products.add(report);
    });
    return products;
  }
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          customer.when(data: (customers) {
                            return TotalCountWidget(
                              title: lang.S.of(context).newCusotmers,
                              icon: MdiIcons.packageCheck,
